...
chkListItem = ['ID', 'PWD', 'EMAIL', 'PHONE']
chkListMsg = ['^ 영문 숫자 한글 중 5자이상인 ID 선택 하세요.\n', 
		'^ 영문 시작, 숫자, 특수기호[$, _, ! , @, #, %, ^ & ~] 포함한 7자 이상 PWD 선택 하세요.\n', 
		'^ word@wrd.com 형식에 맞지 않습니다.\n', 
		'^ 숫자만 입력 가능합니다.\n']
menuChk = []
userList=[]
temp=[]
...
def regExChk(itemChk, inputChk):
		if itemChk == "ID":
				...
				regChk=patt1.match(inputChk)
				print(regChk)
				return regChk
		elif itemChk =="PWD":
				patt2=re.compile(r'[a-z|A-Z]{1,}[0-9]{1,}[$_!@#%^&*~]{1,}|[a-z|aA-Z]{1,}[$_!@#%^&*~]{1,}[0-9]{1,}')
				if len(inputChk)>=7:
					regChk=patt2.match(inputChk)
					print(regChk)
					return regChk
		elif itemChk =="EMAIL":
				patt3=re.compile(r'[\w]{0,}[@]{1,}[\w]{1,}[.]{1,}[\a-z|A-Z]')
				regChk=patt3.match(inputChk)
				print(regChk)
				return regChk
		elif itemChk =="PHONE":
				patt4=re.compile(r'[\d]{1,}')
				regChk=patt4.match(inputChk)
				print(regChk)
				return regChk
...
def memIns():
	...
	for signUp in range(len(itemList)):
		...
		templist=input()
		if itemList[signUp] in chkListItem:
			while True:
				if regExChk(itemList[signUp], templist):
					break
				else:
					idxChk=chkListItem.index(itemList[signUp])
					print(f"\n\t\t{chkListMsg[idxChk]}")
					print(f"\t{itemList[signUp]:<10}:",end="") 
					templist=input()
